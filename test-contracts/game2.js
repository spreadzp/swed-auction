...
describe('GAME test suite', async function () {
    ...
    let seedDev2 = "pact fine flavor cry cargo glance junior burger various fruit soup inside dumb total hood";
   ...
   it('3 start the game', async function () {
    ...
    it('14 Should be winner up bet', async function () {
       
        const lastRoundWinnersState = await accountDataByKey("lastRoundWinner", address(seedDev2));
        console.log('lastRoundWinnersState.value :', lastRoundWinnersState.value);
        const lastRoundWinner1 = await accountDataByKey( lastRoundWinnersState.value + address(seedGamer1), address(seedDev2));
        console.log('!!!!! lastRoundWinner1 :', lastRoundWinner1);
        const lastRoundWinner2 = await accountDataByKey( lastRoundWinnersState.value + address(seedGamer2), address(seedDev2));
        console.log('??????? lastRoundWinner2 :', lastRoundWinner2);
        const lastRoundHash = await accountDataByKey("randhash", address(seedDev2));
        console.log('$$$$ $$$$$$lastRoundHash :', lastRoundHash.value);
    })
    it('15 Should success forwardRound when to call  owner address', async function () {
        // this.timeout(60000);
        const numberRoundState = await accountDataByKey("numberRound", address(seedDev2));
        console.log('###########numberRoundState :', numberRoundState);
        const iTxSet = invokeScript({
        ...
        await waitForTx(iTxSet.id); 
        const state = await stateChanges(iTxSet.id);
        console.log('stateforwardRound :', state.data);
        const numberRoundStateafter = await accountDataByKey("numberRound", address(seedDev2));
        console.log('###########numberRoundStateafter :', numberRoundStateafter);
        
    })
   

    it('16 Should be the game stopped after forwardRound', async function () {
        const numberRoundState = await accountDataByKey("numberRound", address(seedDev2));
        console.log('###########numberRoundState :', numberRoundState);
        const startState = await accountDataByKey("startGame", address(seedDev2));
        expect(startState.value).to.equal(false);
    })
    it('17 start the game', async function () {
        const numberRoundState = await accountDataByKey("numberRound", address(seedDev2));
        console.log('########### numberRound before :', numberRoundState);
        const iTxSet = invokeScript({
            dApp: address(seedDev2),
            fee: 0.09 * wvs,
            call: {
        ...
        const state = await stateChanges(iTxSet.id)
        const numberRoundState1 = await accountDataByKey("numberRound", address(seedDev2));
        console.log('###########numberRound after :', numberRoundState1);
    }) 

    it('18 Should be the game started', async function () {
        const startState = await accountDataByKey("startGame", address(seedDev2));
        expect(startState.value).to.equal(true);
    })
   
    it('19 make Bet More', async function () {
        const numberRoundState = await accountDataByKey("numberRound", address(seedDev2));
        const iTxSet = invokeScript({
            dApp: address(seedDev2),
            fee: 0.04 * wvs,
    ...
    it('20 make Bet Less', async function () {
        const numberRoundState = await accountDataByKey("numberRound", address(seedDev2));
        console.log('###########numberRoundState :', numberRoundState);
